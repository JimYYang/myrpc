...
void ZkClient::Start()
{
    ...
    std::string host = *config.getConfig("zookeeper.ip");
    std::string port = *config.getConfig("zookeeper.port");
...
void ZkClient::Create(const char *path, const char *data, int datalen, int state)
{
    ...
    if (ZNONODE == flag) // 表示path的znode节点不存在
    {
        ...
        else
        {
            ...
            spdlog::error("znode create error... path: {}.", path);
            exit(EXIT_FAILURE);
        }
    }
}
...
std::string ZkClient::GetData(const char *path)
{
    ...
    if (flag != ZOK)
    {
        spdlog::error("get znode error... path: {}.", path);
        return "";
    }
    else
    {
